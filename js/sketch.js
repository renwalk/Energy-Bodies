/* ============================================================================
   ENERGY BODIES — DISPLAY SKETCH (fixed globals + single session)
   Date: 2025-11-06
   What changed (summary):
   • Removed the bootstrap shim that was racing with load order.
   • Bound global Display APIs (beginSession, onPrint, start/stopTracking) ONCE at top-level.
   • Use one global session: window.__ebSession (never a second local copy).
   • Minor clean-up in TUNE.coupling and defensive guards.
   ============================================================================ */
// --- Energy Bodies: hard-bind display APIs immediately (top of file) ---
(function(){
  // One global session for the whole page
  window.__ebSession = window.__ebSession || null;

  // Begin averaging; if classes not ready, keep retrying
  function __beginSessionRetry(){
    if (typeof SessionAverager !== 'function') {
      // Wait for the class to be parsed, then try again
      setTimeout(__beginSessionRetry, 40);
      return;
    }
    if (!window.__ebSession) window.__ebSession = new SessionAverager();
    window.__ebSession.begin();
    console.log('[SESSION] begin (hard-bound)');
  }

  // Expose the API right now (so Display can see it), and let it self-resolve
  window.beginSession = function(){ __beginSessionRetry(); };

  // Print handler that prefers the averaged session, with snapshot fallback
  window.onPrint = function(){
    try {
      if (window.__ebSession && typeof window.__ebSession.end === 'function') {
        const avg = window.__ebSession.end();
        console.log('[SESSION] end → samples:', avg?.samples ?? 0);
        if (avg && avg.samples > 0 && typeof finalizeAndPrint === 'function') {
          finalizeAndPrint(avg, { bgMode:'black', size:'4x6', dpi:300 });
          return;
        }
      }
    } catch (e) {
      console.warn('[SESSION] print error, snapshot fallback', e);
    }
    if (typeof window.onPrintClick === 'function') window.onPrintClick();
  };
})();
...
/* =============================
   ...
   ============================= */
console.log('✅ sketch.js (fixed) loading...');
...
const POSE_EMIT_MS = 80; // ~12.5 fps

// SINGLE global session instance (always referenced via window.__ebSession)
window.__ebSession = window.__ebSession || null;
...
let regionOffsets = {}; for (const r of regionNames) regionOffsets[r] = { x:0, y:0 };
...
/* =============================
   2) SMALL UTILS & AVERAGERS
   ============================= */
class OnlineMean { constructor(){ this.mean=0; this.n=0; } add(x){ if(!Number.isFinite(x)) return; this.n++; this.mean += (x - this.mean)/this.n; } }
class OnlineMeanArray { constructor(len=0){ this.means=new Array(len).fill(0); this.n=0; } add(arr){ if(!arr||!arr.length) return; if(this.means.length!==arr.length) this.means=new Array(arr.length).fill(0); this.n++; for(let i=0;i<arr.length;i++){ const x=arr[i]; if(!Number.isFinite(x)) continue; this.means[i]+= (x - this.means[i])/this.n; } } }
class TimeWeightedMean { constructor(){ this.sumWX=0; this.sumW=0; } add(x,w){ if(!Number.isFinite(x)||!Number.isFinite(w)||w<=0) return; this.sumWX += x*w; this.sumW += w; } mean(){ return this.sumW ? (this.sumWX/this.sumW) : 0; } }

class SessionAverager {
  constructor(){ this.reset(); }
  reset(){
    this.active=false; this.lastTs=0; this.samples=0;
    this.structure=new OnlineMean(); this.balance=new OnlineMean(); this.posture=new OnlineMean(); this.velocity=new OnlineMean();
    this.emotions={ anxiety:new OnlineMean(), sadness:new OnlineMean(), joy:new OnlineMean(), anger:new OnlineMean(), fear:new OnlineMean(), calm:new OnlineMean() };
    this.regionWidths=new OnlineMeanArray(6); this.segmentProfile=new OnlineMeanArray(segmentProfile?.length||0);
    this.timeWeighted=false;
  }
  begin(){ this.reset(); this.active=true; this.startTs=performance.now(); this.lastTs=this.startTs; }
  add(sample){ if(!this.active) return; const now=performance.now(); const dt=Math.max(1, now-this.lastTs); this.lastTs=now; const addS=(om,x)=> (this.timeWeighted && om.add.length===2) ? om.add(x,dt) : om.add(x);
    addS(this.structure, sample.structure??0); addS(this.balance, sample.balance??0); addS(this.posture, sample.posture??0); addS(this.velocity, sample.velocity??0);
    if(sample.emotions) for(const k in this.emotions) addS(this.emotions[k], sample.emotions[k]??0);
    if(sample.regionWidths) this.regionWidths.add(sample.regionWidths);
    if(sample.segmentProfile) this.segmentProfile.add(sample.segmentProfile);
    this.samples++; }
  end(){ const out={ durationMs: performance.now()-this.startTs, samples:this.samples, structure:this.structure.mean, balance:this.balance.mean, posture:this.posture.mean, velocity:this.velocity.mean, emotions:{}, regionWidths:this.regionWidths.means.slice(), segmentProfile:this.segmentProfile.means.slice() }; for(const k in this.emotions) out.emotions[k]=this.emotions[k].mean; this.active=false; return out; }
}

function clamp01(x){ return Math.max(0, Math.min(1, x)); }


// Will be bound to real functions after declarations (below)
...
function setup(){
  ...
  // Session instance (also kept in window.__ebSession for router access)
  ...
  video.size(640,480); video.elt.playsInline = true; video.hide();
  ...
  poseNet.on('pose', results => {
    ...
    const now = millis();
    if (now - __lastPoseEmitAt >= POSE_EMIT_MS) { __lastPoseEmitAt = now; emitPoseToControl(pose); }
  });
...
function draw(){
  ...
  const vals = {}; for (let n of emotionNames) vals[n]=emotionSliders[n].value();
  const fearAmt=vals.fear/5, sadnessAmt=vals.sadness/5, joyAmt=vals.joy/5, angerAmt=vals.anger/5, anxietyVal=vals.anxiety, calmAmt=vals.calm/5;

  const FV=TUNE.visuals.fear; const baseScale=lerp(FV.baseScaleFrom,FV.baseScaleTo,fearAmt); const fearFreq=lerp(FV.freqFrom,FV.freqTo,fearAmt); const pulse=FV.pulseAmp*Math.sin(frameCount*fearFreq); const fearScale=baseScale+pulse;
  ...
  const totalBaseHeight=baseRegionSpacings.reduce((a,b)=>a+b,0); const maxBodyHeight=height*0.7; const bodyHeightScale=maxBodyHeight/totalBaseHeight; const regionSpacings=baseRegionSpacings.map(s=>s*bodyHeightScale);
  ...
  const maskImage=shapeMask.get(); const emotionImage=emotionGraphics.get(); const patternImage=patternGraphics.get();
  ...
  push(); noStroke(); fill(255); textSize(14); text(`[${displayOrientation}]`,12,40); pop();
}

/* =============================
   5) PRINT FLOW (AVERAGE→SNAPSHOT)
   ============================= */
function onPrintAverageThenSnapshot(){
  const s = window.__ebSession;
  if (!s) { console.warn('[SESSION] none; snapshot fallback'); return onPrintSnapshotFallback(); }
  const avg = s.end();
  console.log('[SESSION] end → samples:', avg.samples, 'duration(ms):', Math.round(avg.durationMs));
  if (!avg || !avg.samples) { console.warn('[SESSION] 0 samples; snapshot fallback'); return onPrintSnapshotFallback(); }
  ...
  const saved = {
    ...
    structure: smoothedStructure, balance: smoothedBalance, posture: smoothedPostureLean, velocity: movementVelocity,
  };

  for (const n of (emotionNames||[])) emotionSliders[n]?.value(avg.emotions?.[n] ?? 0);
  for (let i=0;i<(regionNames||[]).length;i++){ const n = regionNames[i]; regionSliders[n]?.value(avg.regionWidths?.[i] ?? 0); }
  regionSliders['spine']?.value(0);
  smoothedStructure = avg.structure ?? 0; smoothedBalance = avg.balance ?? 0; smoothedPostureLean = avg.posture ?? 0; movementVelocity = avg.velocity ?? 0;

  requestAnimationFrame(() => {
    const dataURL = captureEnergyBodyHiRes();

    for (let i=0;i<(emotionNames||[]).length;i++){ const n=emotionNames[i]; emotionSliders[n]?.value(saved.emotions[i]); }
    for (let i=0;i<(regionNames||[]).length;i++){ const n=regionNames[i]; regionSliders[n]?.value(saved.regions[i]); }
    regionSliders['spine']?.value(saved.spine);
    smoothedStructure=saved.structure; smoothedBalance=saved.balance; smoothedPostureLean=saved.posture; movementVelocity=saved.velocity;
...
function updatePoseFactors(pose){
  ...
  if(__prevKeypoints){
    ...
    let sum=0,n=0;
    for(const k of kp){ const prev=__prevKeypoints.find(p=>p.part===k.part); if(!prev) continue; const dx=(k.position.x-prev.position.x)/diag; const dy=(k.position.y-prev.position.y)/diag; sum+=Math.hypot(dx,dy); n++; }
    if(n>0) velAvg=sum/n;
  }
  ...
  let structure=0; if(Ls&&Rs&&Lw&&Rw){ const span=dist(Ls.x,Ls.y,Rs.x,Rs.y); const lw=dist(Ls.x,Ls.y,Lw.x,Lw.y)/span; const rw=dist(Rs.x,Rs.y,Rw.x,Rw.y)/span; structure=clamp01(map((lw+rw)*0.5, 0.7,2.0, 0,1)); }
  smoothedStructure=lerp(smoothedStructure,structure,0.15);

  const shoulderDiff=Math.abs(Ls?.y - Rs?.y), hipDiff=Math.abs(pose.leftHip?.y - pose.rightHip?.y);
  smoothedBalance=lerp(smoothedBalance, (shoulderDiff+hipDiff), 0.15);

  let lean=0; if(Ls&&Rs&&pose.leftHip&&pose.rightHip){ const sx=(Ls.x+Rs.x)/2, sy=(Ls.y+Rs.y)/2; const hx=(pose.leftHip.x+pose.rightHip.x)/2, hy=(pose.leftHip.y+pose.rightHip.y)/2; const ang=Math.atan2(hy-sy, hx-sx)-0; lean=clamp01(map(Math.abs(ang), 0.0, Math.PI/6, 0,1)); }
...
function coupleRegionsToPose(pose){
  ...
  if(ls && rs){
    ...
    const spineVal = map(spineSway, -TUNE.coupling.spine.swayRange, TUNE.coupling.spine.swayRange, 0, regionMaxWidths.spine||100);
    regionSliders['spine'].value( lerp(regionSliders['spine'].value(), spineVal, TUNE.coupling.spine.lerp) );
  }
}

/* =============================
   9) POSE TRANSFORM / ANCHOR
   ============================= */
...
function emitPoseMetrics(){
  if (trackingStarted && window.__ebSession?.active) {
    ...
    if (p && p.score > 0.25) window.__ebSession.add(buildSampleForThisFrame());
  }
  const now = millis(); if (now - __lastEmitMs < 80) return; __lastEmitMs = now;
  if (window.EnergyBodiesDisplay){
    ...
    EnergyBodiesDisplay.pose({ movementVelocity, fastVel, structure: smoothedStructure, balance: normBalance, postureLean: normPosture, avgY: normAvgY });
  }
}

let __lastEcho = 0;
function maybeEchoState(){
  const t = millis(); if (!window.EnergyBodiesDisplay) return; if (t - __lastEcho < 250) return; __lastEcho = t;
...
function resetAll({ echo = true, resetPoseCaches = true, stopTrackingNow = true } = {}) {
  ...
  movementVelocity = 0; fastVel = 0; slowVel = 0; smoothedStructure=0; smoothedBalance=0; smoothedPostureLean=0; smoothedAvgY=0;
  followPose=false; followAxis='both'; poseRot=0; poseSc=1; const vw=video?.width||640, vh=video?.height||480; poseTx=vw/2; poseTy=vh/2; _poseSeenAt = millis ? millis() : 0;
  if (resetPoseCaches) { __prevKeypoints=null; if (__prevByPart?.clear) __prevByPart.clear(); poses=[]; }
  scene?.clear?.(); patternGraphics?.clear?.(); emotionGraphics?.clear?.(); shapeMask?.clear?.();
...
function buildSampleForThisFrame(){
  const emotions = {}; for (const n of emotionNames) emotions[n] = Number(emotionSliders[n]?.value() || 0);
  const regionWidths = regionNames.map(n => Number(regionSliders[n]?.value() || 0));
  return { structure: smoothedStructure, balance: smoothedBalance, posture: smoothedPostureLean, velocity: movementVelocity, emotions, regionWidths, segmentProfile: segmentProfile.slice() };
}
